...
zooAnimals.map(function (item) {
  lowCaseAnimalNames.push(`pattern: ${item.animal_name.toLocaleLowerCase()}`);
});
...
zooAnimals.filter((item) => {
  if (item.population < 5) {
    lowPopulationAnimals.push(`Animal: ${item.animal_name}`);
  }
});
...
populationTotal = zooAnimals.reduce((accumalter, curruntvalue) => {
  return (accumalter += curruntvalue.population);
}, populationTotal);
